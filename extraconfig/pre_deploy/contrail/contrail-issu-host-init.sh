...
if [[ "$role_short" != "contrailcontrollerissu" && \
      "$role_short" != "contraildpdk" && \
      "$role_short" != "novacompute" && \
      "$role_short" != "contrailtsn" && \
  ]]; then
...
ssh-copy-id -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null localhost
